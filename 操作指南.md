...
### 步骤 4：生成1万条随机测试数据（符合题目要求）

**方法一：使用SQL脚本生成（推荐，已集成到Docker）**
1. 输入以下命令生成1万条随机销售数据：
   ```bash
   docker exec -i $(docker ps -q -f name=mysql) mysql -u root -prootpassword < generate_random_data.sql
   ```
2. 生成过程可能需要一些时间，请耐心等待
3. 完成后，你应该能看到已生成10000条记录的确认信息

**方法二：使用Python脚本生成（可选）**
1. 确保已安装Python 3
2. 运行以下命令生成CSV格式的随机数据：
   ```bash
   cd db
   python3 generate_random_data.py
   ```
3. 生成完成后，会在db目录下创建sales_data.csv文件
4. 可以通过LOAD DATA INFILE命令将CSV数据导入MySQL数据库

**方法三：使用Mockaroo生成（推荐，符合题目要求）**
按照题目要求，您也可以使用Mockaroo生成随机数据：
...
3. 运行Mockaroo数据生成脚本：
   ```bash
   cd db
   python3 generate_data_with_mockaroo.py
   ```

使用说明：
- 您需要先在 https://mockaroo.com/ 注册账号获取API密钥
- 将API密钥替换到脚本中的`YOUR_MOCKAROO_API_KEY`位置
- 运行脚本后会生成`sales_data_mockaroo.csv`文件
- 使用提供的MySQL命令导入数据到数据库

如果Mockaroo API有调用限制，也可以直接访问Mockaroo网站手动创建数据集并下载CSV文件。
