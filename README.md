# 销售分析系统 - 后端技术文档

## 📚 项目概述

这是一个基于 Spring Boot + MyBatis + MySQL 的销售数据分析后端系统，为编程初学者设计，提供完整的从开发到部署的学习案例。

**主要功能特点：**
- 完整的销售数据 CRUD 操作（增删改查）
- 强大的销售数据聚合查询功能
- 批量数据导入能力
- 集成 Swagger/OpenAPI 文档
- Docker 容器化部署
- 完整的单元测试覆盖
- 性能优化和索引设计

## 🛠 技术栈

- **编程语言**: Java 1.8
- **框架**: Spring Boot 2.7.x
- **ORM 框架**: MyBatis 3.5.x
- **数据库**: MySQL 8.0
- **容器化**: Docker & Docker Compose
- **API 文档**: Swagger/OpenAPI
- **测试框架**: JUnit 5, Mockito

## 📁 项目结构

```
houduanB/
├── src/
│   ├── main/
│   │   ├── java/com/salesanalysis/
│   │   │   ├── Application.java               # Spring Boot 应用入口
│   │   │   ├── controller/                    # 控制器层
│   │   │   │   └── SalesController.java       # REST API 控制器
│   │   │   ├── service/                       # 业务逻辑层
│   │   │   │   ├── SalesService.java          # 服务接口
│   │   │   │   └── impl/SalesServiceImpl.java # 服务实现
│   │   │   ├── mapper/                        # MyBatis 映射器
│   │   │   │   └── SalesMapper.java           # 数据访问接口
│   │   │   ├── model/                         # 数据模型
│   │   │   │   ├── Sales.java                 # 销售数据模型
│   │   │   │   └── SalesSummary.java          # 销售汇总模型
│   │   │   ├── config/                        # 配置类
│   │   │   │   └── SwaggerConfig.java         # Swagger 配置
│   │   │   └── util/                          # 工具类
│   │   │       └── DateUtils.java             # 日期工具类
│   │   └── resources/
│   │       ├── mapper/SalesMapper.xml         # MyBatis XML 映射文件
│   │       ├── application.properties         # Spring Boot 配置
│   │       └── static/                        # 静态资源
│   └── test/                                  # 单元测试
│       └── java/com/salesanalysis/            # 测试代码
├── db/                                        # 数据库脚本
│   ├── init.sql                               # 数据库初始化脚本
│   ├── import_data.sql                        # 测试数据导入脚本
│   └── generate_random_data.py                # 随机数据生成脚本
├── scripts/                                   # 脚本工具
│   └── test_and_performance_analysis.sh       # 测试和性能分析脚本
├── Dockerfile                                 # Docker 构建文件
├── docker-compose.yml                         # Docker Compose 配置
├── pom.xml                                    # Maven 项目配置
└── README.md                                  # 项目文档
```

## ⚙️ 环境准备

### 1. 安装 Docker Desktop

Docker 是一个容器化平台，可以帮助你在隔离的环境中运行应用程序。

**MacOS 安装步骤：**
1. 访问 [Docker 官方网站](https://docs.docker.com/desktop/install/mac-install/)
2. 下载适合 M1 Pro 芯片的 Docker Desktop
3. 安装完成后，启动 Docker Desktop
4. 确保 Docker 服务正在运行（右上角会有 Docker 图标）

### 2. 验证 Docker 安装

打开终端，输入以下命令检查 Docker 是否正常工作：

```bash
docker --version
docker-compose --version
```

如果能看到版本信息，说明安装成功！

## 🚀 快速开始

### 1. 克隆项目

如果这是从 GitHub 克隆的项目，使用以下命令：

```bash
git clone [项目地址] cd houduanB
```

如果是直接下载的项目文件，直接进入项目目录即可。

### 2. 使用 Docker Compose 启动项目

在项目根目录下执行以下命令：

```bash
docker-compose up -d
```

这个命令会：
- 下载 MySQL 和所需的 Docker 镜像
- 启动 MySQL 数据库服务
- 构建并启动 Spring Boot 应用
- 创建必要的数据卷和网络配置

### 3. 初始化数据库

执行数据库初始化脚本：

```bash
# 进入 db 目录
cd db

# 执行初始化脚本
docker exec -i $(docker ps -q -f name=mysql) mysql -u root -prootpassword < init.sql

# 导入测试数据
docker exec -i $(docker ps -q -f name=mysql) mysql -u root -prootpassword < import_data.sql
```

### 4. 验证服务启动

服务启动后，可以通过以下方式验证：

- **Spring Boot 应用**：访问 http://localhost:8080
- **Swagger API 文档**：访问 http://localhost:8080/swagger-ui.html
- **数据库连接**：使用 MySQL 客户端连接 localhost:3306

## 🔍 API 接口文档

系统提供以下 RESTful API 接口：

### 1. 获取所有销售数据

```
GET /api/sales
```

**响应示例：**
```json
[
  {
    "id": 1,
    "saleDate": "2023-06-15T00:00:00.000+0000",
    "product": "产品A",
    "region": "华东",
    "amount": 1299.99
  }
]
```

### 2. 按 ID 获取销售数据

```
GET /api/sales/{id}
```

**参数：**
- `id`: 销售记录 ID

### 3. 创建销售记录

```
POST /api/sales
```

**请求体：**
```json
{
  "saleDate": "2023-06-15",
  "product": "新产品",
  "region": "华北",
  "amount": 999.99
}
```

### 4. 更新销售记录

```
PUT /api/sales/{id}
```

**参数：**
- `id`: 销售记录 ID

**请求体：**
```json
{
  "id": 1,
  "saleDate": "2023-06-16",
  "product": "更新产品",
  "region": "华南",
  "amount": 1599.99
}
```

### 5. 删除销售记录

```
DELETE /api/sales/{id}
```

**参数：**
- `id`: 销售记录 ID

### 6. 获取销售汇总数据

```
GET /api/sales/summary
```

**查询参数：**
- `date` (可选): 按日期筛选 (格式: YYYY-MM-DD)
- `product` (可选): 按产品筛选
- `region` (可选): 按地区筛选

**响应示例：**
```json
[
  {
    "date": "2023-06-15",
    "product": "产品A",
    "region": "华东",
    "totalAmount": 1299.99,
    "count": 1
  }
]
```

### 7. 批量导入销售数据

```
POST /api/sales/batch
```

**请求体：**
```json
[
  {
    "saleDate": "2023-06-20",
    "product": "批量产品1",
    "region": "批量地区1",
    "amount": 599.99
  },
  {
    "saleDate": "2023-06-21",
    "product": "批量产品2",
    "region": "批量地区2",
    "amount": 799.99
  }
]
```

## 📊 数据库说明

### 数据库表结构

**sales 表：**

| 字段名 | 数据类型 | 约束 | 描述 |
|-------|---------|------|------|
| id | INT | PRIMARY KEY AUTO_INCREMENT | 销售记录ID |
| sale_date | DATE | NOT NULL | 销售日期 |
| product | VARCHAR(255) | NOT NULL | 产品名称 |
| region | VARCHAR(255) | NOT NULL | 销售地区 |
| amount | DECIMAL(10,2) | NOT NULL | 销售金额 |

### 索引设计

系统在以下字段上创建了索引，以优化查询性能：
- `idx_sale_date` (sale_date): 加速日期筛选
- `idx_product` (product): 加速产品筛选
- `idx_region` (region): 加速地区筛选

## 🧪 运行测试

### 1. 运行单元测试

在项目根目录执行：

```bash
# 使用 Maven 运行测试
mvn test
```

### 2. 运行功能测试和性能分析

使用提供的测试脚本：

```bash
# 运行测试和性能分析脚本
./scripts/test_and_performance_analysis.sh
```

## 💻 开发指南

### 本地开发

如果你想在本地开发环境中运行项目，需要：

1. 确保已安装 Java 1.8 和 Maven
2. 启动本地 MySQL 数据库
3. 修改 `application.properties` 中的数据库连接信息
4. 运行以下命令：

```bash
mvn spring-boot:run
```

### 生成大量测试数据

可以使用提供的 Python 脚本生成随机测试数据：

```bash
# 安装依赖
pip install random datetime csv

# 运行数据生成脚本
python db/generate_random_data.py

# 导入生成的 CSV 数据到数据库
mysql -u root -p salesdb < import_csv.sql
```

## 📈 性能优化

### 索引优化

系统已在常用查询字段上创建了索引，但在处理大量数据时，可以考虑：

1. 创建复合索引：如果经常同时按日期和产品查询，可以创建复合索引
2. 定期优化表：使用 `OPTIMIZE TABLE sales;` 命令优化表空间
3. 监控慢查询：配置 MySQL 慢查询日志，分析性能瓶颈

### 查询优化

1. 避免全表扫描，尽量使用索引字段进行筛选
2. 合理设置查询条件，避免不必要的数据加载
3. 对于聚合查询，确保正确使用 GROUP BY 和索引

## 🚢 部署说明

### 使用 Docker Compose 部署

```bash
docker-compose up -d
```

### 环境变量配置

可以通过修改 docker-compose.yml 中的环境变量来自定义配置：

- `MYSQL_ROOT_PASSWORD`: MySQL 根密码
- `MYSQL_DATABASE`: 数据库名称
- `SPRING_DATASOURCE_URL`: 数据库连接 URL
- `SPRING_DATASOURCE_USERNAME`: 数据库用户名
- `SPRING_DATASOURCE_PASSWORD`: 数据库密码

## 🛡 安全注意事项

1. 生产环境中请修改默认密码
2. 限制数据库访问权限
3. 考虑添加 API 认证机制
4. 定期更新依赖库以修复安全漏洞

## 📋 检查清单

### 项目功能验证

- [ ] 数据库表是否成功创建
- [ ] 测试数据是否成功导入
- [ ] Spring Boot 应用是否正常启动
- [ ] Swagger 文档是否可访问
- [ ] 所有 API 接口是否正常工作
- [ ] 聚合查询功能是否正确
- [ ] 性能分析结果是否符合预期

### 常见问题排查

1. **服务无法启动**：检查端口是否被占用，Docker 是否正常运行
2. **数据库连接失败**：检查数据库配置和网络连接
3. **API 调用失败**：查看应用日志，检查错误信息
4. **性能问题**：检查索引使用情况，优化查询语句

## 🤝 贡献指南

欢迎对项目进行贡献！如果你发现任何问题或有改进建议，请：

1. Fork 项目
2. 创建功能分支
3. 提交更改
4. 发起 Pull Request

## 📝 许可证

本项目仅供学习使用。

## 📮 联系方式

如有任何问题，请随时联系。
